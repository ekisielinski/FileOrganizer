...
namespace FileOrganizer.CommonUtils
{
    public static class Guard
    {
        ...
        public static long MinValue( long value, long min, string? paramName = null )
        {
            if (value >= min) return value;
        ...
        public static int InRange( int value, int min, int max, string? paramName = null )
        {
            if (min > max) throw new ArgumentOutOfRangeException
                    ( nameof( min ), $"The {nameof( min )} parameter cannot be greater than the {nameof( max )} parameter" );
